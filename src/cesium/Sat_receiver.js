...
import { useCesiumStore, useRecordStore } from '@/store';
...
function sendObserver() { 
  ...
  if (receiverIcon) {
    receiverIcon.position = obsPos;
    receiverIcon.show = iReceiver.value;
  }
  if ( iReceiver.value ) {
    if (receiveLine) {
...
async function loadSat() {
  ...
  icons = viewer.scene.primitives.add(new Cesium.BillboardCollection());
  receiverIcon = icons.add({
    position: obsPos,
    image: "/images/receiver.png",
    scale: 2,
    minPixelSize: 32,
    maxPixelSize: 64,
    scaleByDistance: new Cesium.NearFarScalar(1e3, 1., 5.0e7, 0.3),
...
function cleanSat() {
  ...
  if (icons) { 
    if (receiverIcon) {
      icons.remove(receiverIcon);
      receiverIcon = null;
    }
    viewer.scene.primitives.remove(icons);
  }
...
export { fetchSatellite, loadSat, setupPicking, tickSending, cleanSat, setSatView,
    pfTune, applyStyle, orbitMinutes, iReceiver, satReceiver, sendObserver, clearOrbits }