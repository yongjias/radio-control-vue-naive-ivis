...
import gltf0 from '@/assets/models/df.gltf.json'
...
export async function gltf_instancing(scene, instances, base, scale) {
  if (instances.length == 0) {
    return null;
  }
  let gltf = JSON.parse(JSON.stringify(gltf0))
  ...
  let fShaderStrs = [
    ...
    "    int featureId = fsInput.featureIds.instanceFeatureId_0;"
  ]
  ...
  for (let i=0; i<instances.length; i++) {
    ...
    instanceIdStrings.push(
      JSON.stringify({
        ...
        status: status==3 ? '异常' : status==2 ? '运行' : status==1 ? '空闲' : '离线',
      })
    )
    // color for manage/control
    if (instances[i].status==3 ) {
        if (i==0) {
            fShaderStrs.push("    if (featureId == 0) {")
            fShaderStrs.push("        material.diffuse = vec3(1.0, 0.08, 0.576);")
        } else {
            fShaderStrs.push("    } else if (featureId == " + i + ") {")
            fShaderStrs.push("        material.diffuse = vec3(1.0, 0.08, 0.576);")
        }
    } else if (instances[i].status==2 ) {
        if (i==0) {
            fShaderStrs.push("    if (featureId == 0) {")
            fShaderStrs.push("        material.diffuse = vec3(0.94, 0.63, 0.38);")
        } else {
            fShaderStrs.push("    } else if (featureId == " + i + ") {")
            fShaderStrs.push("        material.diffuse = vec3(0.94, 0.63, 0.38);")
        }
    } else if (instances[i].status==1 ) {
        if (i==0) {
            fShaderStrs.push("    if (featureId == 0) {")
            fShaderStrs.push("        material.diffuse = vec3(0.0, 1.0, 0.0);")
        } else {
            fShaderStrs.push("    } else if (featureId == " + i + ") {")
            fShaderStrs.push("        material.diffuse = vec3(0.0, 1.0, 0.0);")
        }
        ...
        if (i==0) {
            fShaderStrs.push("    if (featureId == 0) {")
            //fShaderStrs.push("        material.diffuse = vec3(0.9, 0.82, 0.8);")
            fShaderStrs.push("        material.diffuse = vec3(0.23, 0.9, 0.8);")
        } else {
            fShaderStrs.push("    } else if (featureId == " + i + ") {")
            //fShaderStrs.push("        material.diffuse = vec3(0.9, 0.82, 0.8);")
            fShaderStrs.push("        material.diffuse = vec3(0.23, 0.9, 0.8);")
  ...
  fShaderStrs.push("   }")
  fShaderStrs.push("}")
  const fShader = fShaderStrs.join("\n")
  //console.log(fShader)
  const customShader = new Cesium.CustomShader({
      fragmentShaderText: fShader
  });
  ...
  let modelInstance = await Cesium.Model.fromGltfAsync({
    ...
    silhouetteColor: Cesium.Color.WHITE.withAlpha(0.1),
    silhouetteSize: 2,
    scene: scene,
...
export function camera_height2D(viewer) {
  const scene = viewer.scene;
  const camera = scene.camera;

  if (scene.mode === Cesium.SceneMode.SCENE3D) {
    // 防御：误用时退回 3D 计算方式
    return camera_height(viewer);
  }
