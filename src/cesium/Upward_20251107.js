...
const update_dfLines = (dfLines) => {
  for (const { mfid, lines } of dfLines) {
...
function findSiteMfidByCoord(sites, lon, lat, toleranceMeters = 30) {
  ...
  function process_upward_record(dataStr, gid) {
    ...
    if (type === 'Direction') {
      ...
      if (lonChanged || latChanged) {
        ...
        if (sInfo) {
          throttledUpdateSitePos(sInfo, { lon, lat, alt: 0 });
        }
...
export { unloadUpward, wsUpward, iTargetShow, switch_power_upward, findSiteMfidByCoord,
  update_lineTarget, del_targets, update_tdoaTarget, update_dfLines };