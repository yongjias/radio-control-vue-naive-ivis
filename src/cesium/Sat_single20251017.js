...
function sendObserver() { 
  ...
  if ( iReceiver.value ) {
    ...
    if (pointsRef[recordStore.connectedSatIdx] && pointsRef[recordStore.connectedSatIdx].color) {
      pointsRef[recordStore.connectedSatIdx].color = colorConnect;
      pointsRef[recordStore.connectedSatIdx].pixelSize = Math.max(10, pfTune.pixelSize+3);
    }
  } else {
    polylines.remove(receiveLine);
    receiveLine = null;
    if (pointsRef[recordStore.connectedSatIdx] && pointsRef[recordStore.connectedSatIdx].color) {
      pointsRef[recordStore.connectedSatIdx].color = colorNormal;
      pointsRef[recordStore.connectedSatIdx].pixelSize = pfTune.pixelSize;
    }
  }
...
async function loadSat() {
  ...
  const devDownwards = Object.values(siteStore.sites).filter(s => s.name === '下行');
  if (devDownwards.length > 0) {
    unWatchReceiver = watch(() => devDownwards[0], (newVal) => {
      if (!newVal) return;
      satReceiver.lat = newVal.lat;
      satReceiver.lon = newVal.lon;
      satReceiver.altMeters = newVal.alt;
      sendObserver();
      ...
      for(let j = 0; j < points.length; j += batchSize) {
        ...
        for(let i = j; i < end; i++) {
          ...
          if (buf[idxBuf] !== 0 || buf[idxBuf + 1] !== 0 || buf[idxBuf + 2] !== 0) {
            ...
            // 连线接收设备和最近的卫星
            if (receiveLine && recordStore.connectedSatIdx === i) {
              receiveLine.positions = [pointsRef[i].position, obsPos];
              if (recordStore.iConnectedSatChange) {
                pointsRef[i].color = colorConnect;
                pointsRef[i].pixelSize = Math.max(10, pfTune.pixelSize+3);
                if (recordStore.previousSatIdx && recordStore.previousSatIdx !== i) {
                  pointsRef[recordStore.previousSatIdx].color = colorNormal;
                  pointsRef[recordStore.previousSatIdx].pixelSize = pfTune.pixelSize;
                }
                recordStore.iConnectedSatChange = false;
              }
            }
  ...
  const picking_sat = (movement) => {
    ...
    if (picked && picked.primitive && picked.primitive.id && picked.primitive instanceof Cesium.PointPrimitive) {
      ...
      if (pointsRef[idx].id.iSelected) {
        ...
        pointsRef[idx].color = idx==recordStore.connectedSatIdx ? colorConnect : colorNormal;
        pointsRef[idx].pixelSize = idx==recordStore.connectedSatIdx ? Math.max(10, pfTune.pixelSize+3) : pfTune.pixelSize;
      }
      // 已存在就删除
      if (selected.has(idx)) {
