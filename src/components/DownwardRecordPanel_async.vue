...
<template>
  ...
  <div ref="elBox" :style="style" class="box" v-if="iOpen">
    ...
    <div class="flex px-10 pt-5 pb-10 justify-between items-center">
      <div class="flex f-c-c gap-5 text-gray-200" >
        ...
        <n-slider v-model:value="timeRange" range :format-tooltip="formatTooltip" 
          :min="tMin" :max="tMax" :step="1000" class="w-350" @dragend="reload_record_time_change"/>
      </div>
      <n-select :options="taskOptions" @update:value="task_filter" 
        size="small" class="w-100" :default-value="-1"/>
      <n-button size="small" @click="filter_hex_area" >
        <i class="i-fe:area-custom text-14" />
        {{ iDragArea ? '取消区域范围' : '设定区域范围' }}
      </n-button>
      <n-button size="small" @click="reload_record_fresh()" >
    ...
    <NDataTable
      ...
      @update:page="reload_record"
      @update:sorter="reload_record_sort"
      :scroll-x="2000"
    ...
    <div class="flex mt--25 pl-5 pb-5">
      ...
      <n-select :options="pageSizeOptions" v-model:value="pagination.pageSize"
        size="tiny" class="w-70 " @update:value="reload_record()" />
    </div>
  </div>
</template>
...
let hexMap = new Map(); // 用于批量添加时的计数映射
...
const reload_record_sort = (sorters) => {
  const sorter = sorters.filter(el => el.columnKey==='time');
  if (sorter.length > 0) {
    tableOrder.value = sorter[0].order;
    reload_record(pagination.page);
  }
}

const reload_record_fresh = async(curPage=1) => {
  reset_current_hex();

  // clear timeRange to reload from scratch
  timeRange.value = []
  await reload_record(curPage);
}

const reload_record_time_change = async(curPage=1) => {
  reset_current_hex();
  await reload_record(curPage);
}

const reset_current_hex = () => {
  const hexIdsToRemove = Array.from(hexMap.keys());
  remove_hex_batch(hexIdsToRemove);
  hexMap = new Map(); // clear hexMap
  selectedKeys.value = [];
}

const reload_record = async(curPage=1) => {
  loading.value = true
  if (timeRange.value.length<2) {
    //const recordsCountResp = await api.downward_getAllCount();
    const recordsTimeRangeResp = await api.downward_getAllTimeRange();
    // set timeRange
    timeRange.value = [Date.parse(recordsTimeRangeResp.data.start), Date.parse(recordsTimeRangeResp.data.end)];
    tMin.value = timeRange.value[0];
    tMax.value = timeRange.value[1];
  }

  let recordResp = null;
  if (iDragArea.value && draggedAreaLons.length>2 && draggedAreaLats.length>2) {
    // filtered by dragged area
    recordResp = await api.downward_getNthChunkInTimeRangeAreaWithMeta({       // use planar index in memory
    //recordResp = await api.downward_getNthChunkInTimeRangeAreaWithMetaDb({   // use spatial index in DB
      startTime: Math.min(...timeRange.value), 
      endTime: Math.max(...timeRange.value), 
      n: curPage, 
      chunkSize: pagination.pageSize,
      lons: draggedAreaLons,
      lats: draggedAreaLats,
      order: tableOrder.value.toLowerCase() === 'ascend' ? 'ASC' : 'DESC',
    });
  } else {
    recordResp = await api.downward_getNthChunkInTimeRangeWithMeta({
      startTime: Math.min(...timeRange.value), 
      endTime: Math.max(...timeRange.value), 
      n: curPage, 
      chunkSize: pagination.pageSize,
  ...
  const records = recordResp.data.data;
  if (records.length>0) {
  ...
  // table pagination
  pagination.itemCount = recordResp.data.total;
...
const update_tableByArea = (lons, lats) => {
  if (lons.length > 0) {
    reset_current_hex();
    // filtered by dragged area
    draggedAreaLons = lons;
    draggedAreaLats = lats;
    reload_record(); 
    reload_record_fresh();
  } 
}
