...
import { useSiteStore, useAppStore } from '@/store';
import { createViewer, AmapMercatorTilingScheme, map_geoJSON_China } from '@/cesium'
...
export const useCesiumStore = defineStore('cesiumStore', () => {
  ...
  function init(container) {
    ...
    viewer = markRaw(createViewer(container))
  ...
  const change_baseMap = async (providerName) => {
    ...
    if (providerName=='OfflineMap') {
        ...
        baseMap = viewer.imageryLayers.addImageryProvider(mapProvide)
        ...
        baseMap = viewer.imageryLayers.addImageryProvider(imageryProviders[providerName])
        //viewer.terrainProvider = terrain
    }
  ...
  const show_map_label = (iShow) => {
    if (mapLabel != undefined && !iShow) {
        // 去除地图标记
        viewer.imageryLayers.remove(mapLabel)
        mapLabel = undefined
    }
    if (mapLabel == undefined && iShow) {
        // 加地图标记
        mapLabel = viewer.imageryLayers.addImageryProvider(imageryProviders.TianDTMap_w)
    }
  }
  ...
  const setBoundingView = (boundingSpheres, pitch=-45, rangeFactor = 10, screenOffsetRatio = 0.2) => {
    ...
    // 使用飞行动画
    viewer.camera.viewBoundingSphere(combinedBoundingSphere, 
      new Cesium.HeadingPitchRange(heading, pitchRad, range),
    );
    if (offsetMeters !== 0) {
      viewer.camera.moveUp(offsetMeters);
    }
    viewer.camera.lookAtTransform(Cesium.Matrix4.IDENTITY)  // unlock camera
  ...
  const setSatView = () => {
    viewer.camera.setView({
      destination: i3D.value ? Cesium.Cartesian3.fromDegrees(116, 39, 1.5e7) : Cesium.Cartesian3.fromDegrees(116, 0, 3.68e7),
      //duration: 1,
      orientation: {
          heading : Cesium.Math.toRadians(0.0), // 方向
          pitch : Cesium.Math.toRadians(-90.0),// 倾斜角度
  ...
  const setView = () => {
    ...
    if (appStore.nearView === '下行' || appStore.nearView === '总体') {
        boundingSpheres.push(siteStore.downwardBoundingSphere);
    }
    if (appStore.nearView === '上行' || appStore.nearView === '总体') {
        boundingSpheres.push(siteStore.upwardBoundingSphere);
    }
    if (boundingSpheres.length === 1 && boundingSpheres[0] && boundingSpheres[0].radius < 10000) {
      boundingSpheres[0].radius = 10000;
    }
    if (appStore.iFarView) {
      // pitch, rangeFactor, screenOffsetRatio
      setBoundingView(boundingSpheres, -45, 13, 0.1);
    } else {
      setBoundingView(boundingSpheres, -90, 11, 0.03);
    }
  }
  ...
  const hlj_border = async() => {
    ...
    viewer.dataSources.add(bd_hlj_detail);
    ...
    viewer.dataSources.add(bd_china);
  }
  */
  ...
  const hlj_border = () => {
    if (!outlineBorder || !mapBorder) {
      ...
      if (!outlineBorder) {
        outlineBorder = viewer.scene.primitives.add(outlinePolygon);
      }
      if (!mapBorder) {
        mapBorder = viewer.scene.primitives.add(fillPolygon);
      }
    }
  }
  let cnOutlineBorder = null;
  const cn_border = async () => { // 1. 加上 async
    if (!cnOutlineBorder) {
      // 2. 加上 await，且不需要传参数了
      const { outlinePolygon } = await map_geoJSON_China(); 
      if (outlinePolygon) {
          cnOutlineBorder = viewer.scene.primitives.add(outlinePolygon);
      }
    }
  }
  ...
  function estimateMapLevel(viewer, tileWidth = 256) {
    const { camera, canvas } = viewer.scene;
    const { latitude: lat, height: h } = camera.positionCartographic; 
    
    // 1. 计算每像素代表的米数 (核心公式)
    // - 3D: PerspectiveFrustum 有 fov
    // - 2D: Orthographic frustum 没有 fov（否则会 NaN）
    const canvasHeight = (canvas?.clientHeight || canvas?.height || 0);
    const canvasWidth = (canvas?.clientWidth || canvas?.width || 0);
    if (!canvasHeight || !canvasWidth) {
      return { level: 0, barWidth: 0, barLabel: '' };
    }

    let metersPerPixel;
    const frustum = camera.frustum;

    if (Cesium.defined(frustum?.fov)) {
      // 3D 透视相机：按高度 + fov 估算地面分辨率
      metersPerPixel = (2 * Math.tan(frustum.fov * 0.5) * h) / canvasHeight;
    } else if (
      Cesium.defined(frustum?.right) && Cesium.defined(frustum?.left)
    ) {
      // 2D 正交相机：frustum 的 left/right 是投影平面上的“米”，用宽度/像素得到投影米/像素
      // 为复用下面的 level 公式（它假设 metersPerPixel 是“地面米/像素”），这里乘 cos(lat) 做纬度修正
      const projectedMetersPerPixel = (frustum.right - frustum.left) / canvasWidth;
      metersPerPixel = projectedMetersPerPixel * Math.cos(Math.abs(lat));
    } else if (
      Cesium.defined(frustum?.top) && Cesium.defined(frustum?.bottom)
    ) {
      // 兜底：用高度方向估算
      const projectedMetersPerPixel = (frustum.top - frustum.bottom) / canvasHeight;
      metersPerPixel = projectedMetersPerPixel * Math.cos(Math.abs(lat));
    } else {
      return { level: 0, barWidth: 0, barLabel: '' };
    }
    ...
    // 纬度修正：在高纬度地区，经度圈变小，分辨率变化
    const resolutionAtLat = metersPerPixel / Math.cos(Math.abs(lat)); 
    let level = Math.floor(Math.log2(circumference / (resolutionAtLat * tileWidth)));
    level = Cesium.Math.clamp(level, 0, 22);

    // --- 计算比例尺 (新增逻辑) ---
    const targetPixelWidth = 100; // 我们希望比例尺大概这么宽
    const rawDistance = metersPerPixel * targetPixelWidth;

    // 找一个“整洁”的距离数值 (1, 2, 5, 10, 20, 50...)
    let distance = 0;
    let label = '';
    ...
    if (rawDistance >= 1000) {
      ...
      // 找最近的整数级别
      const base = Math.pow(10, Math.floor(Math.log10(km)));
      const candidate = Math.floor(km / base); 
      // 归一化到 1, 2, 5
      let factor = 1;
      if (candidate >= 5) factor = 5;
      else if (candidate >= 2) factor = 2;
      
      const finalKm = base * factor;
      distance = finalKm * 1000;
      label = finalKm + ' km';
    } else {
      // 小于 1km，用米
      const base = Math.pow(10, Math.floor(Math.log10(rawDistance)));
      const candidate = Math.floor(rawDistance / base);
      let factor = 1;
      if (candidate >= 5) factor = 5;
      else if (candidate >= 2) factor = 2;
    ...
    return { level, barWidth, barLabel: label };
  }

  function toggleFPS() {
    const show = !viewer.scene.debugShowFramesPerSecond
    viewer.scene.debugShowFramesPerSecond = show
  }

  const iPausedMouseEvents = ref(false);

  return { init, getViewer, toggleFPS, change_map_range, i3D, setView, setSatView,
    change_baseMap, show_map_label, switch_home3d, go_north_up, estimateMapLevel,
